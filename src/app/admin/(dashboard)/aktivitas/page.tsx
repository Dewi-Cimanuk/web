...
export const metadata = { title: "Kelola Aktivitas | Admin DEWI" };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Manajemen Aktivitas</h2>
          <p className="text-slate-500 text-sm">Kelola data Aktivitas desa.</p>
        </div>
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <Link href={"/admin/aktivitas/" + item.id + "/edit"}>
                      ...
                      <form action={async () => {
                        "use server";
                        const { deleteActivity } = await import("@/modules/activity/services/actions");
