...
import { updateCulinary } from "@/modules/culinary/services/actions";
import prisma from "@/lib/prisma";

export const metadata = { title: "Edit Kuliner | Admin DEWI" };

export default async function EditKulinerPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const item = await prisma.culinary.findUnique({ where: { id: id } }) as any;
  if (!item) notFound();

  const updateAction = updateCulinary.bind(null, item.id);

  return (
    <div className="space-y-6 w-full">
      <div className="flex items-center gap-4">
        <Link href="/admin/kuliner">
          <Button variant="ghost" size="icon" className="rounded-full">
            <ArrowLeft className="w-5 h-5" />
          </Button>
        </Link>
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Edit Kuliner</h2>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        <form action={updateAction} className="space-y-6">
          ...
          <div className="space-y-2"><label htmlFor="address" className="text-sm font-semibold text-slate-700">Alamat</label><input type="text" id="address" name="address" defaultValue={item.address || ""} className="w-full h-10 px-3 rounded-lg border border-slate-200 text-sm focus:border-primary outline-none" /></div><div className="space-y-2"><label htmlFor="openHours" className="text-sm font-semibold text-slate-700">Jam Buka</label><input type="text" id="openHours" name="openHours" defaultValue={item.openHours || ""} className="w-full h-10 px-3 rounded-lg border border-slate-200 text-sm focus:border-primary outline-none" /></div>
          ...
          <div className="pt-4 border-t border-slate-100 flex justify-end gap-3">
            <Link href="/admin/kuliner"><Button type="button" variant="outline" className="rounded-xl">Batal</Button></Link>
