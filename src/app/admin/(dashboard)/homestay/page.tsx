...
export const metadata = { title: "Kelola Homestay | Admin DEWI" };

export default async function AdminHomestayPage() {
  const items = await prisma.accommodation.findMany({ orderBy: { createdAt: "desc" } });

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Manajemen Homestay</h2>
          <p className="text-slate-500 text-sm">Kelola data Homestay desa.</p>
        </div>
        <Link href="/admin/homestay/create">
          <Button className="bg-primary hover:bg-primary/90 text-white rounded-xl flex items-center gap-2">
            <Plus className="w-4 h-4" /> Tambah Homestay
          </Button>
        </Link>
      </div>
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <Link href={"/admin/homestay/" + item.id + "/edit"}>
                      ...
                      <form action={async () => {
                        "use server";
                        const { deleteAccommodation } = await import("@/modules/accommodation/services/actions");
